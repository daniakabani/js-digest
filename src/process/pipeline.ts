...
export async function processTopN(storage: Storage, topN = 5): Promise<{ processed: ProcessedItem[] }> {
  const items = storage.listItems();
  const ranked = rank(dedupe(items)).slice(0, topN);
  const provider: LLMProvider = process.env.LLM_MOCK === "true" || !process.env.OPENAI_API_KEY ? new MockLLM() : new OpenAIProvider();
